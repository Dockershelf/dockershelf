...
def update_debian(basedir):
    ...
    dockerfile_url_holder = "https://github.com/Dockershelf/dockershelf/blob/master/debian/{0}/Dockerfile"
    pulls_badge_holder = "https://img.shields.io/docker/pulls/dockershelf/debian?colorA=22313f&colorB=4a637b"
    pulls_url_holder = "https://hub.docker.com/r/dockershelf/debian"
    size_badge_holder = (
        "https://img.shields.io/docker/image-size/dockershelf/debian/{0}.svg?colorA=22313f&colorB=4a637b"
    )
    ...
    debian_readme_tablelist_holder = "|[`{0}`]({1})|[![]({2})]({3})|[![]({4})]({5})|[![]({6})]({7})|"
    ...
    for debian_version_long, debian_suite in debian_versions:
        ...
        if debian_version_long == "sid":
            matrix.append(matrix_str_main.format(debian_version_long, debian_suite, "latest"))
            tag_matrix.extend([debian_version_long, debian_suite, "latest"])
        else:
            matrix.append(matrix_str.format(debian_version_long, debian_suite))
        ...
        debian_dockerfile_content = re.sub("%%DEBIAN_RELEASE%%", debian_version_long, debian_dockerfile_content)
        debian_dockerfile_content = re.sub("%%DEBIAN_SUITE%%", debian_suite, debian_dockerfile_content)
    ...
    debian_readme_content = re.sub("%%DEBIAN_TABLE%%", debian_readme_table, debian_readme_template_content)
