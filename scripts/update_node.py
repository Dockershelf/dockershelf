...
from .config import debian_versions, node_versions
...
def update_node(basedir):
    ...
    dockerfile_url_holder = "https://github.com/Dockershelf/dockershelf/blob/master/node/{0}/Dockerfile"
    pulls_badge_holder = "https://img.shields.io/docker/pulls/dockershelf/node?colorA=22313f&colorB=4a637b"
    pulls_url_holder = "https://hub.docker.com/r/dockershelf/node"
    size_badge_holder = "https://img.shields.io/docker/image-size/dockershelf/node/{0}.svg?colorA=22313f&colorB=4a637b"
    ...
    node_readme_tablelist_holder = "|[`{0}`]({1})|[![]({2})]({3})|[![]({4})]({5})|[![]({6})]({7})|"
    ...
    for node_version_long in node_versions:
        for debian_version in [debian_versions_eq["stable"], debian_versions_eq["unstable"]]:
            logger.info("Processing Node {0} ({1})".format(node_version_long, debian_version))
            ...
            dockerfile_badge = dockerfile_badge_holder.format(node_version_long, debian_version)
            ...
            node_dockerfile_content = re.sub(
                "%%BASE_IMAGE%%", base_image.format(debian_version), node_dockerfile_content
            )
            node_dockerfile_content = re.sub("%%DEBIAN_RELEASE%%", debian_version, node_dockerfile_content)
            node_dockerfile_content = re.sub("%%NODE_VERSION%%", node_version_long, node_dockerfile_content)
            node_dockerfile_content = re.sub("%%NODE_DEBIAN_SUITE%%", debian_version, node_dockerfile_content)
    ...
    node_readme_content = re.sub("%%NODE_TABLE%%", node_readme_table, node_readme_template_content)
