...
from .config import debian_versions, python_versions
...
def update_python(basedir):
    ...
    dockerfile_url_holder = "https://github.com/Dockershelf/dockershelf/blob/master/python/{0}/Dockerfile"
    pulls_badge_holder = "https://img.shields.io/docker/pulls/dockershelf/python?colorA=22313f&colorB=4a637b"
    pulls_url_holder = "https://hub.docker.com/r/dockershelf/python"
    size_badge_holder = (
        "https://img.shields.io/docker/image-size/dockershelf/python/{0}.svg?colorA=22313f&colorB=4a637b"
    )
    ...
    python_readme_tablelist_holder = "|[`{0}`]({1})|[![]({2})]({3})|[![]({4})]({5})|[![]({6})]({7})|"
    ...
    for python_version_long in python_versions:
        for debian_version in [debian_versions_eq["stable"], debian_versions_eq["unstable"]]:
            logger.info("Processing Python {0} ({1})".format(python_version_long, debian_version))
            ...
            dockerfile_badge = dockerfile_badge_holder.format(python_version_long, debian_version)
            ...
            if python_version_long == latest_version:
                if debian_version == "sid":
                    matrix.append(
                        matrix_str_latest_unstable.format(
                            python_version,
                            python_version_unstable,
                            python_version_long,
                            f"{latest_major_version}-unstable",
                            f"{latest_major_version}-sid",
                    ...
                    matrix.append(
                        matrix_str_latest_stable.format(
                            python_version,
                            python_version_stable,
                            f"{latest_major_version}-stable",
                            f"{latest_major_version}-{debian_version}",
                            "latest-stable",
                            f"latest-{debian_version}",
                        )
                    )
                    tag_matrix.extend(
                        [
            ...
            python_dockerfile_content = re.sub(
                "%%BASE_IMAGE%%", base_image.format(debian_version), python_dockerfile_content
            )
            python_dockerfile_content = re.sub("%%DEBIAN_RELEASE%%", debian_version, python_dockerfile_content)
            python_dockerfile_content = re.sub("%%PYTHON_VERSION%%", python_version_long, python_dockerfile_content)
            python_dockerfile_content = re.sub("%%PYTHON_DEBIAN_SUITE%%", debian_version, python_dockerfile_content)
    ...
    python_readme_content = re.sub("%%PYTHON_TABLE%%", python_readme_table, python_readme_template_content)
