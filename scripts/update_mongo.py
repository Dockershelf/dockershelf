...
def update_mongo(basedir):
    ...
    pulls_badge_holder = ('https://img.shields.io/docker/pulls/dockershelf/mongo'
                          '?colorA=22313f&colorB=4a637b&cacheSeconds=900')
    pulls_url_holder = ('https://hub.docker.com/r/dockershelf/mongo')
    size_badge_holder = ('https://img.shields.io/docker/image-size/'
                         'dockershelf/mongo/{0}.svg'
                         '?colorA=22313f&colorB=4a637b&cacheSeconds=900')
    size_url_holder = ('https://hub.docker.com/r/dockershelf/mongo')
    matrix_latest_str = (
    ...
    for mongo_version in mongo_versions:
        ...
        pulls_badge = pulls_badge_holder.format(mongo_version)
        pulls_url = pulls_url_holder.format(mongo_version)
        size_badge = size_badge_holder.format(mongo_version)
        size_url = size_url_holder.format(mongo_version)
