...
from .config import debian_versions, go_versions
...
def update_go(basedir):
    ...
    dockerfile_url_holder = "https://github.com/Dockershelf/dockershelf/blob/master/go/{0}/Dockerfile"
    pulls_badge_holder = "https://img.shields.io/docker/pulls/dockershelf/go?colorA=22313f&colorB=4a637b"
    pulls_url_holder = "https://hub.docker.com/r/dockershelf/go"
    size_badge_holder = "https://img.shields.io/docker/image-size/dockershelf/go/{0}.svg?colorA=22313f&colorB=4a637b"
    ...
    go_readme_tablelist_holder = "|[`{0}`]({1})|[![]({2})]({3})|[![]({4})]({5})|[![]({6})]({7})|"
    ...
    for go_version_long in go_versions:
        ...
        for debian_version in [debian_versions_eq["stable"], debian_versions_eq["unstable"]]:
            logger.info("Processing Go {0} ({1})".format(go_version_short, debian_version))
            ...
            dockerfile_badge = dockerfile_badge_holder.format(go_version_short, debian_version)
            ...
            go_dockerfile_content = re.sub("%%BASE_IMAGE%%", base_image.format(debian_version), go_dockerfile_content)
            go_dockerfile_content = re.sub("%%DEBIAN_RELEASE%%", debian_version, go_dockerfile_content)
            go_dockerfile_content = re.sub("%%GO_VERSION%%", go_version_long, go_dockerfile_content)
            go_dockerfile_content = re.sub("%%GO_DEBIAN_SUITE%%", debian_version, go_dockerfile_content)
    ...
    go_readme_content = re.sub("%%GO_TABLE%%", go_readme_table, go_readme_template_content)
